...
onBeforeMount(fetchClient);

async function fetchClient() {
    const loader = await loadingService.show()
    try {
        const clientData = await clientService.getClient(parseInt(String(route.params.clientId)));
        clientForm.cin = clientData.data.cin;
        clientForm.email = clientData.data.email;
        clientForm.pin = clientData.data.pin;
        clientForm.name = clientData.data.name;
...
async function updateClient() {
    const loader = await loadingService.show()
    try {
        await clientService.updateClient(client.value!.id,clientForm);
...
<template>
    ...
    <header class="container-fluid py-3 py-lg-4 d-flex justify-content-between align-items-center">
        <div class="d-flex justify-content-between w-100">
            <h1 class="m-0 fw-semibold">Edit client</h1>
    ...
    <section class="mid-section d-flex flex-column">
        <div class="mid-info container-fluid py-4">
            <div class="mt-3 row">
                ...
                <div class="mb-3 mt-4">
                    <input class="btn btn-primary" type="submit" value="Update Client" @click.prevent="updateClient()"
                        :disabled="v$.$invalid">
                </div>
            </div>
        </div>
